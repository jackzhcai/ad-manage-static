<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'编辑直客广告优先级':'新增直客广告优先级'">
      <el-form :model="form" label-width="100px" label-position="left">
        ...
        <el-form-item label="优先级列表">
          <el-select v-model="form.priorityLevel" style="width: 100%" placeholder="请选择优先级列表">
            <el-option label="小米信息流" :value="1" />
            <el-option label="奇虎信息流" :value="2" />
          </el-select>
        </el-form-item>
      </el-form>
...
import { fetchOptimizeDirectList } from '@/api/new/optimize'
...
export default {
  ...
  methods: {
    async getList() {
      ...
      const res = await fetchOptimizeDirectList(this.listQuery)
